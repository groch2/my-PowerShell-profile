...
New-Variable -Name my_scripts_directory -Value (Join-Path $env:HOMEPATH 'Documents\WindowsPowerShell\MyScripts') -Visibility Private -Option Constant
function git-get-local-branches-not-in-distant-repo {
  $script = Join-Path -Path $my_scripts_directory -ChildPath 'git_get_local_branches_not_in_distant_repo.ps1'
  & $script
}
...
function get-all-files-in-directory-recurse($directory) {
  $script = Join-Path -Path $my_scripts_directory -ChildPath 'get_all_files_in_directory_recurse.ps1'
  & $script -directory $directory
}
function git-save-branches-with-current-head-commit {
  $script = Join-Path -Path $my_scripts_directory -ChildPath 'git_save_branches_with_current_head_commit.ps1'
  & $script $directory
}
